...
export default function ReleaseTimeline({ items, isLive }) {
  const data = isLive && items?.length
    ? items.map(a => ({
        date:   a.aired ? new Date(a.aired).toLocaleDateString('en-US', { month:'short', day:'numeric' }) : 'TBA',
        title:  a.title,
        status: a.airing ? 'premiere' : 'confirmed',
      }))
    : TIMELINE;
    ...
    <div>
      ...
      <div style={{ padding:`20px ${PAD} 50px`, overflowX:'auto' }} className="no-scrollbar">
        <div style={{ position:'relative', minWidth:640, height:140 }}>
          ...
          {data.map((ev, i) => {
            const left = `${(i / Math.max(data.length - 1, 1)) * 96}%`;
            const up = i % 2 === 0;
            ...
            return (
              <div key={ev.title + i} style={{ position:'absolute', left, top:0, bottom:0, display:'flex', flexDirection:'column', alignItems:'center' }}>
                {up ? <div style={{ marginBottom:8 }}>{label}</div> : <div style={{ height:50 }}/>}
                <div style={{ width:11, height:11, borderRadius:50, background:color, border:'2px solid var(--bg)', boxShadow:`0 0 0 1px ${color}`, marginTop: up ? 0 : 'auto', marginBottom: up ? 'auto' : 0 }}/>
